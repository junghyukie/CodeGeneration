#!/bin/bash
# Round-2 inference (disagree_explore) for step_6: tasks up to and including step 6.
#
...
#   OUTPUT_DIR          - where round-2 results are saved   (default: ./inference_results/round2_disagree_explore_routing_topp_1.0/step_6)
#   TASKS               - comma-separated language list     (default: python,cpp,swift,rust,csharp,java,php)
#   CUDA_DEVICE         - GPU index                         (default: 1)
#   ROUND_NUM           - round number for output filenames (default: 2)
#
# Usage:
#   bash scripts/executable/round2/infer_round2_step6.sh
...
: "${OUTPUT_DIR:=./inference_results/round2_disagree_explore_routing_topp_1.0/step_6}"
: "${TASKS:=python,cpp,swift,rust,csharp,java,php}"
: "${CUDA_DEVICE:=1}"
: "${ROUND_NUM:=2}"
...
MAX_PROMPT_LENS="4096,4096,4096,4096,4096,4096,4096"
MAX_ANS_LENS="2048,2048,2048,2048,2048,2048,2048"

echo "[round2_step6] ============================================"
echo "[round2_step6] Model          : $MODEL"
echo "[round2_step6] Base adapter   : $BASE_PATH"
echo "[round2_step6] Router         : $ROUTER_PATH"
echo "[round2_step6] Prev results   : $PREV_RESULTS_DIR ($PREV_RESULTS_SOURCE)"
echo "[round2_step6] TB router      : $TB_ROUTER_PATH"
echo "[round2_step6] Output dir     : $OUTPUT_DIR"
echo "[round2_step6] Tasks          : $TASKS"
echo "[round2_step6] ============================================"
...
python infer_gmm.py \
  ...
  --prev_results_subfolder step_6 \
  --round_num             "$ROUND_NUM" \
  --traceback_router_path "$TB_ROUTER_PATH" \
  --pass_through_correct \
  --pad_predictions_to    5

echo "[round2_step6] Done. Results saved to $OUTPUT_DIR"

HF_REPO="ankhanhtran02/$(basename "$(dirname "$OUTPUT_DIR")")"
echo "[round2_step6] Uploading $OUTPUT_DIR → $HF_REPO"
