#!/bin/bash
# Round-2 inference (disagree_explore) for step_4: tasks up to and including step 4.
#
...
#   OUTPUT_DIR          - where round-2 results are saved   (default: ./inference_results/round2_disagree_explore_routing_topp_1.0/step_4)
#   TASKS               - comma-separated language list     (default: python,cpp,swift,rust,csharp)
#   CUDA_DEVICE         - GPU index                         (default: 1)
#   ROUND_NUM           - round number for output filenames (default: 2)
#
# Usage:
#   bash scripts/executable/round2/infer_round2_step4.sh
...
MAX_PROMPT_LENS="4096,4096,4096,4096,4096"
MAX_ANS_LENS="2048,2048,2048,2048,2048"

echo "[round2_step4] ============================================"
echo "[round2_step4] Model          : $MODEL"
echo "[round2_step4] Base adapter   : $BASE_PATH"
echo "[round2_step4] Router         : $ROUTER_PATH"
echo "[round2_step4] Prev results   : $PREV_RESULTS_DIR ($PREV_RESULTS_SOURCE)"
echo "[round2_step4] TB router      : $TB_ROUTER_PATH"
echo "[round2_step4] Output dir     : $OUTPUT_DIR"
echo "[round2_step4] Tasks          : $TASKS"
echo "[round2_step4] ============================================"
...
python infer_gmm.py \
  ...
  --prev_results_subfolder step_4 \
  --round_num             "$ROUND_NUM" \
  --traceback_router_path "$TB_ROUTER_PATH" \
  --pass_through_correct \
  --pad_predictions_to    5

echo "[round2_step4] Done. Results saved to $OUTPUT_DIR"

HF_REPO="ankhanhtran02/$(basename "$(dirname "$OUTPUT_DIR")")"
echo "[round2_step4] Uploading $OUTPUT_DIR → $HF_REPO"
