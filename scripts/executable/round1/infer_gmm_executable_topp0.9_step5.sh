#!/usr/bin/env bash
# Round-1 GMM-router inference: step 5.
...
python infer_gmm.py \
  ...
  --inference_model_path "python/0","cpp/0","swift/0","rust/0","csharp/0","java/0" \
  --router_weight_path   router/ckpt_executable_dim256_comp4_vf0.001_mean \
  --benchmark            executable \
  --routing_mode         "${ROUTING_MODE}" \
  --routing_temperature  1.0 \
  --max_prompt_len       2048,2048,2048,2048,2048,2048 \
  --max_ans_len          2048,2048,2048,2048,2048,2048 \
  --inference_batch      1 \
  --inference_tasks      python,cpp,swift,rust,csharp,java \
  --do_sample \
  --temperature          0.2 \
  --top_p                0.95 \
  --routing_top_p        0.9 \
