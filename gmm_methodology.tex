...
\subsection{Mean-Pool Representation}
...
The routing representation is the full-sequence mean pool:
\begin{equation}
    \tilde{\mathbf{h}} = \operatorname{pool}(\mathbf{H},\, \mathbf{m}) \in \mathbb{R}^d.
    \label{eq:hfull}
\end{equation}
...
\begin{equation}
    \mathbf{h} = \operatorname{LN}(\tilde{\mathbf{h}}) \in \mathbb{R}^{d}, \qquad
...
A \emph{fixed} projection matrix $\mathbf{P} \in \mathbb{R}^{p \times d}$ ($p \leq d$, default $p=256$) reduces dimensionality for efficient density estimation.
The rows of $\mathbf{P}$ are constructed to be orthonormal:

\begin{enumerate}[noitemsep]
    \item Draw $\mathbf{A} \in \mathbb{R}^{d \times p}$ with $A_{ij} \sim \mathcal{N}(0, 1/p)$ using a fixed random seed.
    \item Compute the thin QR factorization: $\mathbf{A} = \mathbf{Q}\mathbf{R}$, $\mathbf{Q} \in \mathbb{R}^{d \times p}$.
...
\begin{tikzpicture}[
    ...
    \node[box, right=1.2cm of enc] (pool) {Mean Pool};
    ...
    \draw[arr] (pool) -- node[above,font=\scriptsize]{$\tilde{\mathbf{h}}\!\in\!\mathbb{R}^{d}$} (ln);
...
\For{$k = 1, \ldots, K$}
    \State \textbf{Extract features:} $Z_k \gets \{\mathbf{z}_i^{(k)} = \mathbf{P}\operatorname{LN}(\mathbf{h}^{(k)}_{\mathrm{full}})\}_{i=1}^{N_k}$
...
\begin{enumerate}
    \item \textbf{Routing feature extraction (Section~\ref{sec:features}).}
          A frozen backbone with full-sequence mean pooling and a fixed QR-orthonormal projection yields stable, task-discriminative routing embeddings $\mathbf{z} \in \mathbb{R}^p$.
