...
class T5ContinualLearner:
    def __init__(self, cfg: TrainConfig):
        ...
        T = len(cfg.task_list)
        self.R_val = [[None for _ in range(T)] for _ in range(T)]  # BLEU
        self.tasks = cfg.task_list
        self.ewc_history: List[Dict[str, Dict[str, torch.Tensor]]] = []
        self.online_fisher: Optional[Dict[str, torch.Tensor]] = None
        self.online_means: Optional[Dict[str, torch.Tensor]] = None
    ...
    def _ewc_named_params(self):
        """Yield (name, param) for trainable parameters (LoRA only)."""
    ...
    def _snapshot_means(self) -> Dict[str, torch.Tensor]:
        means = {}
        for name, p in self._ewc_named_params():
            means[name] = p.detach().clone()
        return means

    def estimate_fisher(self, task: str):
        """Approximate diagonal Fisher using gradients of NLL over a few batches."""
        self.model.eval()
        fisher = {}
        for name, p in self._ewc_named_params():
            fisher[name] = torch.zeros_like(p, device=self.device)
        dl = self.tasks_data[task]["train"]
        n_batches = 0
        for n_batches, batch in enumerate(dl, start=1):
            if n_batches > self.cfg.ewc_estimate_batches:
                break
            batch = {k: v.to(self.device) for k, v in batch.items()}
            self.model.zero_grad(set_to_none=True)
            out = self.model(input_ids=batch["input_ids"], attention_mask=batch["attention_mask"], labels=batch["labels"])  # cross-entropy
            loss = out.loss
            loss.backward()
            for name, p in self._ewc_named_params():
        ...
        if n_batches == 0:
            self.logger.warning("EWC Fisher estimation saw 0 batches; skipping.")
            return None
        for name in fisher:
            fisher[name] /= float(n_batches)
        means = self._snapshot_means()

        if self.cfg.online_ewc:
            if self.online_fisher is None:
                self.online_fisher = {k: v.clone() for k, v in fisher.items()}
                self.online_means = {k: v.clone() for k, v in means.items()}
            else:
                for name in fisher:
                    self.online_fisher[name] = self.cfg.ewc_gamma * self.online_fisher[name] + fisher[name]
                    self.online_means[name] = means[name].clone()
        else:
            self.ewc_history.append({"fisher": fisher, "means": means})
        self.logger.info(
            f"[EWC] Estimated Fisher on task '{task}' over {n_batches} batches (online={self.cfg.online_ewc})."
        )

    def _ewc_penalty(self) -> torch.Tensor:
        if self.cfg.online_ewc:
            if self.online_fisher is None or self.online_means is None:
                return torch.tensor(0.0, device=self.device)
            penalty = 0.0
            for name, p in self._ewc_named_params():
                penalty = penalty + (self.online_fisher[name] * (p - self.online_means[name]) ** 2).sum()
            return penalty
        else:
            if not self.ewc_history:
                return torch.tensor(0.0, device=self.device)
            total = 0.0
            for snap in self.ewc_history:
                F = snap["fisher"]
                M = snap["means"]
                term = 0.0
                for name, p in self._ewc_named_params():
                    term = term + (F[name] * (p - M[name]) ** 2).sum()
                total = total + term
            return total
    ...
    def train_one_task(self, task: str, task_idx: int):
        self.model.set_adapter(self.cfg.shared_adapter_name)
        self.model.enable_adapters()
        self.model.train()
        ...
        for ep in range(self.cfg.epochs):
            for i, batch in enumerate(train_dl, start=1):
                ...
                if task_idx > 0:
                    ...
                    loss = loss + 0.5 * self.cfg.ewc_lambda * penalty
                loss.backward()
                opt.step(); sch.step(); opt.zero_grad(set_to_none=True)
                if (i % K) == 0:
                    self.logger.info(f"[TRAIN] {task} | epoch {ep+1} | step {i}/{len(train_dl)} | loss={loss.item():.4f}")
        self.model.save_pretrained(self.shared_lora_path)
        self.estimate_fisher(task)
    ...
    def train_continual(self):
        ...
        for i, task in enumerate(self.tasks):
            ...
            self.train_one_task(task, task_idx=i)
...
if __name__ == "__main__":
    ...
    p.add_argument("--target_seq_len", type=int, default=256)
    ...
    p.add_argument("--ewc_estimate_batches", type=int, default=64)
    p.add_argument("--online_ewc", action="store_true")
    ...
    cfg = TrainConfig(
        ...
        ewc_estimate_batches=args.ewc_estimate_batches,
